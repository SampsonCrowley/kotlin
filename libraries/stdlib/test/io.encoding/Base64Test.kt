...
class Base64Test {
    ...
    fun index() {
        ...
        for ((base64, scheme) in codecs) {
            ...
            base64.encode(bytes, destination, endIndex = 3)
            assertEquals(symbols.substring(0, 4), destination.toString())
            base64.encode(bytes, destination, startIndex = 3)
        ...
        for ((base64, scheme) in codecs) {
            ...
            assertFailsWith<IndexOutOfBoundsException> { base64.encodeToByteArray(bytes, destination, destinationOffset = -1) }
            assertFailsWith<IndexOutOfBoundsException> { base64.encodeToByteArray(bytes, destination, destinationOffset = destination.size + 1) }
            assertFailsWith<IndexOutOfBoundsException> { base64.encodeToByteArray(bytes, destination, destinationOffset = 1) }

            assertTrue(destination.all { it == 0.toByte() })

            var length = base64.encodeToByteArray(bytes, destination, endIndex = 3)
            assertContentEquals(symbols.encodeToByteArray(0, 4), destination.copyOf(length))
            length += base64.encodeToByteArray(bytes, destination, destinationOffset = length, startIndex = 3)
        ...
        for ((base64, scheme) in codecs) {
            ...
            assertFailsWith<IndexOutOfBoundsException> { base64.decode(symbols, destination, destinationOffset = -1) }
            assertFailsWith<IndexOutOfBoundsException> { base64.decode(symbols, destination, destinationOffset = destination.size + 1) }
            assertFailsWith<IndexOutOfBoundsException> { base64.decode(symbols, destination, destinationOffset = 1) }

            assertTrue(destination.all { it == 0.toByte() })

            var length = base64.decode(symbols, destination, endIndex = 4)
            assertContentEquals(bytes.copyOfRange(0, 3), destination.copyOf(length))
            length += base64.decode(symbols, destination, destinationOffset = length, startIndex = 4)
        ...
        for ((base64, scheme) in codecs) {
            ...
            val destination = ByteArray(6)
            assertFailsWith<IndexOutOfBoundsException> { base64.decode(symbolBytes, destination, destinationOffset = -1) }
            assertFailsWith<IndexOutOfBoundsException> { base64.decode(symbolBytes, destination, destinationOffset = destination.size + 1) }
            assertFailsWith<IndexOutOfBoundsException> { base64.decode(symbolBytes, destination, destinationOffset = 1) }

            assertTrue(destination.all { it == 0.toByte() })

            var length = base64.decode(symbolBytes, destination, endIndex = 4)
            assertContentEquals(bytes.copyOfRange(0, 3), destination.copyOf(length))
            length += base64.decode(symbolBytes, destination, destinationOffset = length, startIndex = 4)
            assertContentEquals(bytes, destination)
        }
    }
