...
public open class Base64 private constructor(
    private val encodeMap: IntArray,
     ...
     */
    public fun <A : Appendable> encode(
        source: ByteArray,
        destination: A,
        startIndex: Int = 0,
        endIndex: Int = source.size
    ...
    /**
     * Decodes symbols from the specified [source] string or its substring.
     * Returns a [ByteArray] containing the resulting bytes.
     *
     * The symbols for decoding are not required to be padded.
     * However, if there is a padding character present, the correct amount of padding character(s) must be present.
     ...
     * The returned byte array is of the size of the resulting bytes.
     *
     * @param source the string to decode symbols from.
     * @param startIndex the beginning (inclusive) of the substring to decode, 0 by default.
     * @param endIndex the end (exclusive) of the substring to decode, length of the [source] string by default.
     *
     * @throws IndexOutOfBoundsException when [startIndex] or [endIndex] is out of range of [source] string indices.
     * @throws IllegalArgumentException when `startIndex > endIndex`.
     * @throws IllegalArgumentException when the symbols for decoding are padded incorrectly.
     *
     * @return a [ByteArray] with the resulting bytes.
     */
    public fun decode(source: String, startIndex: Int = 0, endIndex: Int = source.length): ByteArray {
    ...
    /**
     * Decodes symbols from the specified [source] string or its substring and writes resulting bytes into the [destination] array.
     * Returns the number of bytes written.
     *
     * The symbols for decoding are not required to be padded.
     * However, if there is a padding character present, the correct amount of padding character(s) must be present.
    ...
    public fun decode(
        ...
        val byteSource = charsToBytes(source, startIndex, endIndex)
        return decode(byteSource, destination, destinationOffset)
    }
    ...
    private fun encodeImpl(
        ...
        while (sourceIndex + 2 < endIndex) {
            ...
            destination[destinationIndex++] = encodeMap[bits ushr 18].toByte()
            destination[destinationIndex++] = encodeMap[(bits ushr 12) and 0x3F].toByte()
            destination[destinationIndex++] = encodeMap[(bits ushr 6) and 0x3F].toByte()
            destination[destinationIndex++] = encodeMap[bits and 0x3F].toByte()
        ...
        when (endIndex - sourceIndex) {
            1 -> {
                ...
                destination[destinationIndex++] = encodeMap[bits ushr 6].toByte()
                destination[destinationIndex++] = encodeMap[bits and 0x3F].toByte()
            ...
            2 -> {
                ...
                destination[destinationIndex++] = encodeMap[bits ushr 12].toByte()
                destination[destinationIndex++] = encodeMap[(bits ushr 6) and 0x3F].toByte()
                destination[destinationIndex++] = encodeMap[bits and 0x3F].toByte()
                destination[destinationIndex++] = padSymbol
            }
...
private val base64EncodeMap = intArrayOf(
...
private val base64UrlEncodeMap = intArrayOf(
    65,  66,  67,  68,  69,  70,  71,  72,  73,  74,  75,  76,  77,  78,  79,  80,  /* 0 - 15 */
    81,  82,  83,  84,  85,  86,  87,  88,  89,  90,  97,  98,  99,  100, 101, 102, /* 16 - 31 */
    103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, /* 32 - 47 */
